...
def apply_affinity(hwids: list[str], cpu: int = -1, apply: bool = True) -> None:
    if apply and cpu > -1:
        # Ensure we're always starting with an even number
        cpu_pair_start = (cpu // 2) * 2
        decimal_affinity = (1 << cpu_pair_start) | (1 << (cpu_pair_start + 1))
...
def main() -> int:
    ...
    if cfg.settings.custom_cpus:
        # remove duplicates, sort, and filter to only even CPUs
        benchmark_cpus = sorted(set(cpu for cpu in cfg.settings.custom_cpus if cpu % 2 == 0))

        if not all(0 <= cpu <= cpu_count for cpu in benchmark_cpus):
            LOG_CLI.error("invalid cpus in custom_cpus array")
            return 1
    else:
        # generate list of even CPUs
        benchmark_cpus = list(range(0, cpu_count + 1, 2))

    # Format CPU pairs for display
    cpu_pairs = [f"{cpu}-{cpu+1}" for cpu in benchmark_cpus]
    cpu_pairs_str = ",".join(cpu_pairs)
    ...
    print(
        ...
        Benchmark CPUs           {"All" if not cfg.settings.custom_cpus else cpu_pairs_str}
    ...
    for cpu in benchmark_cpus:
        LOG_CLI.info("benchmarking CPU %d-%d", cpu, cpu+1)
